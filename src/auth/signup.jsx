...
// Function to send OTP to the user
  // This function sends the OTP to the user's email or phone number
  const sendOtp = async (userData) => {
      console.log("sendOtp called with userData");
      console.log("localhost:8000/api/v1/auth/authRoutes/sendOtp calling with data:", userData);
      const otpResponse = await axios.post("http://localhost:8000/api/v1/auth/authRoutes/sendOtp/signup", userData);
...
//handleContinue function to validate input and send OTP
  const handleContinue = async (e) => {
    ...
    try {
      ...
      const res = await axios.post("http://localhost:8000/api/v1/auth/authRoutes/checkUser", userData);
      if (res.data.message === "User already exists") {
        console.log("User already exists, redirecting to sign in");
        setMessage({ type: "warning", text: "User already exists. Please sign in." });
        setTimeout(() => {
        setDefault();
        setOnOpen(false)
        navigate("/signin");
      ...
      console.log("User does not exist, proceeding to send OTP");
    ...
    if (message === 'Invalid email address') {
      setMessage({ type: "warning", text: "Please enter a valid email." });
    } else if (message.includes("Typo")) {
      setMessage({ type: "warning", text: `Did you mean ${err?.response?.data?.suggestion}? Please correct your email.`});
    }  else {
...
// Function to confirm the OTP entered by the user
  const handleConfirmOtp = async (e) => {
    ...
    if (fullOtp.length !== 4) {
      ...
      console.log("Returning from handleConfirmOtp due to invalid OTP length");
      return;
    }
     setLoading(true);
    try {
      const verifyRes = await axios.post("http://localhost:8000/api/v1/auth/authRoutes/verifyOtp", {
        contact: input,
        otp: fullOtp,
      });
...
// Function to handle going back to the previous step
  ...
  const handleFinalSignup = async (e) => {
    e.preventDefault();
    console.log("handleFinalSignup called");
    ...
    try {
      ...
      const signupRes = await axios.post("http://localhost:8000/api/v1/auth/authRoutes/signup", userResponse );
      console.log("Signup response:", signupRes.data);

      if (signupRes.data.message === 'Signup Successful') {
      ...
      setMessage({ type: "failure", text: "Signup failed!" });
    }
    } catch (err) {
      console.error("Signup error:", err);
      setMessage({ type: "failure", text: "Signup failed! Please try again." });
    }
    finally{
      setLoading(false);
    }
  ...
  <>
    ...
    >
      <h2 className="text-3xl font-bold text-center text-pink-500">Sign Up</h2>
  ...
  <>
    ...
    <div className="flex flex-col gap-6 w-full transition-all duration-500 ease-in-out z-10">
      ...
      <div className="text-center text-gray-700 dark:text-gray-300 mb-2">
        ...
        <div className="font-semibold mt-1 text-pink-700">{input}</div>
  ...
  className={`text-sm font-medium ${
    resendTimer > 0 ? 'text-gray-400' : 'text-pink-600 hover:underline'
  }`}
>
  {resendTimer > 0
    ...
    : "Resend OTP"}
        ...
        {/* Submit Button */}
        <Button
          className="w-full mt-2 bg-pink-500 hover:bg-pink-600 text-white font-semibold"
          type="submit"
        >
          Confirm OTP
        </Button>
  ...
  <>
    ...
    <form onSubmit={handleFinalSignup} className="flex flex-col gap-6 w-full transition-all duration-300 z-10">
      <h2 className="text-2xl font-bold text-center mb-4 text-fuchsia-700">Create Your Account</h2>
